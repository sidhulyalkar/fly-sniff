...
V2 = {
    ...
    "fly_sniff.odor_motion_qualification_receipt",
}
...
def test_sealed_v1_does_not_import_v2_analysis_or_qualification():
    source = Path(__file__).resolve().parents[1] / "src" / "fly_sniff"
    violations = {
        name: sorted(_imports(source / name) & V2)
        for name in SEALED
        if _imports(source / name) & V2
    }
    assert not violations, f"sealed v1 imported v2 analysis/qualification modules: {violations}"
