...
def render_v4(
    ...
    *,
    ...
    fig = plt.figure(figsize=(SHOWCASE_WIDTH / SHOWCASE_DPI, SHOWCASE_HEIGHT / SHOWCASE_DPI), dpi=SHOWCASE_DPI, facecolor=BG)
    grid = fig.add_gridspec(5, 1, height_ratios=[0.29, 1.50, 0.49, 0.88, 0.20], hspace=0.11)
    title_ax, room_ax, sensor_ax, mechanism_ax, footer_ax = [fig.add_subplot(grid[i, 0]) for i in range(5)]
    ...
    def draw(video_index: int):
        ...
        reveal = bool(first["found"] or second["found"] or video_index >= max(0, n_video - 2 * fps))

        title_ax.clear(); title_ax.set_facecolor(BG); title_ax.axis("off")
        title_ax.text(0.5, 0.70, config["allowed_headline_now"], ha="center", va="center", fontsize=38, color=TEXT, fontweight="bold")
        title_ax.text(0.5, 0.22, "YOU CAN SEE THE SMELL • THE FLY CAN'T", ha="center", va="center", fontsize=11.4, color=MUTED, fontweight="bold")
        ...
        footer_ax.clear(); footer_ax.set_facecolor(BG); footer_ax.axis("off")
        if reveal:
            ...
            footer_ax.text(0.5, 0.16, "SEALED MECHANISM PROBE • CHASE STILL DEVELOPMENT-ONLY", ha="center", va="center", fontsize=8.0, color=MUTED, fontweight="bold")
        return []

    ani = animation.FuncAnimation(fig, draw, frames=n_video, interval=1000 / fps, blit=False)
    try:
