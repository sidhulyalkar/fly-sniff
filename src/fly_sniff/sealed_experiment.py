...
from . import trained_final
from .candidate_seal import sha256_file as candidate_file_sha256
...
def _verify_matched_binding(
    ...
    if not isinstance(binding, dict):
        raise TypeError("matched training report is not bound to a sealed candidate manifest")
...
def freeze_final_main() -> None:
    ...
    manifest = trained_final.build_trained_final_manifest(
        bundle=bundle,
    ...
    manifest["candidate_manifest_file_sha256"] = candidate_file_sha256(args.candidate_manifest)
...
def final_main() -> None:
    ...
    trained_final.verify_trained_final_manifest(final_manifest)
    ...
    if final_manifest.get("candidate_manifest_file_sha256") != candidate_file_sha256(
        args.candidate_manifest
    ):
        raise ValueError("final manifest candidate-manifest file hash mismatch")
    ...
    lock_payload = {
        ...
        "candidate_manifest_sha256": candidate["manifest_sha256"],
        "candidate_manifest_file_sha256": candidate_file_sha256(args.candidate_manifest),
