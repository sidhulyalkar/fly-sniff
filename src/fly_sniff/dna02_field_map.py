...
def load_authority(path: str | Path = DEFAULT_AUTHORITY) -> dict[str, Any]:
    ...
    if (
        notebook.get("path") != "import_preprocess_data.ipynb"
        or notebook.get("git_blob_sha1") != _EXPECTED_NOTEBOOK_BLOB
    ):
    ...
    if (
        mapping.get("ephys_A", {}).get("secondary_label") != "a2_l"
        or mapping.get("ephys_A", {}).get("soma_side") != "L"
    ):
        raise ValueError("ephys_A must remain mapped to left a2/DNa02")
    if (
        mapping.get("ephys_B", {}).get("secondary_label") != "a2_r"
        or mapping.get("ephys_B", {}).get("soma_side") != "R"
    ):
...
def _inspection_field_metadata(schema_inventory: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    raise ValueError(
        "inspection receipt exposes neither classic MATLAB variables nor HDF5 top-level metadata"
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the frozen DNa02 raw-field authority and optionally confirm an authenticated "
            "schema receipt"
        )
    )
