...
from .recorded_showcase import _agent, _draw_sensor_hud, _outcome_text, _precompute_histories, _validate_social_contract
...
def _validate_inputs(
    ...
    if mechanism.get("phase_mapping_must_remain_unresolved", True):
    ...
    observed_thresholds = set(str(key) for key in e002c.get("threshold_reports", {}))
    ...
    for family in ("FC2A", "FC2B", "FC2C"):
        ...
        threshold_10 = fc2.get("interfaces", {}).get(family, {}).get("threshold_reports", {}).get("10", {})
...
def _draw_mechanism_panel(
    ...
    ax.text(0.02, 0.92, mechanism["label"], color="#FDE68A", fontsize=9.2, fontweight="bold")
    ax.text(
        0.98,
        0.92,
        "MaleCNS v1.0 • modeled state, not measured firing",
        ha="right",
        color=MUTED,
        fontsize=7.0,
    )

    # Two independently constrained structural inputs converge on PFL3. FC2 columns
    # remain anatomical labels only; no physical angle is assigned in this renderer.
    fc2_pos = (0.08, 0.59)
    epg_pos = (0.08, 0.30)
    pfl3_pos = (0.34, 0.45)
    ...
    ax.text(
        0.02,
        0.08,
        "FC2→PFL3: 24/24 targets for A/B/C at w≥10 • phase→angle mapping unresolved",
        color="#C4B5FD",
        fontsize=6.9,
        fontweight="bold",
    )

    # Display the fixed threshold-5 trace while reporting every preregistered threshold.
    x0, x1 = 0.48, 0.98
    y0, y1 = 0.22, 0.77
    ...
    styles = {
        "goal_only": ("goal lane", "#C4B5FD"),
        "heading_only": ("heading lane", "#67E8F9"),
        "joint": ("joint state", "#FDE68A"),
    }
    for name in ("goal_only", "heading_only", "joint"):
        ...
        ax.text(x1, ys[-1], label, ha="right", va="bottom", fontsize=6.2, color=color)

    ax.axvline(xs[probe_index], ymin=y0, ymax=y1, color="#E2E8F0", linewidth=0.8, alpha=0.38)
    ax.text(
        x0,
        0.82,
        f"E002c PFL3 mean |activity| • display trace w≥{threshold} • step {probe_index + 1}/{steps}",
        color=TEXT,
        fontsize=7.0,
        fontweight="bold",
    )
    ...
    badges = []
    for structural_threshold, changed, reachable in _threshold_summary(e002c):
        badges.append(f"w≥{structural_threshold}: {changed}/{reachable}")
    ax.text(
        x0,
        0.08,
        "joint-changed / dual-reachable   " + "   •   ".join(badges),
        color=MUTED,
        fontsize=6.4,
    )
...
def render_recorded_showcase_v3(
    ...
    *,
    ...
    grid = fig.add_gridspec(5, 1, height_ratios=[0.30, 1.48, 0.52, 0.92, 0.20], hspace=0.12)
    ...
    def draw(video_index: int):
        ...
        title_ax.axis("off")
        title_ax.text(0.5, 0.70, "WHO FARTED?", ha="center", va="center", fontsize=38, color=TEXT, fontweight="bold")
        title_ax.text(
            0.5,
            0.22,
        ...
        _draw_mechanism_panel(mechanism_ax, config, e002c, fc2, probe_index)

        footer_ax.clear()
        footer_ax.set_facecolor(BG)
        footer_ax.axis("off")
        if reveal:
            footer_ax.text(0.5, 0.70, f"BUSTED: {PEOPLE[CULPRIT_INDEX][2]}", ha="center", va="center", fontsize=15.0, color="#D9F99D", fontweight="bold")
            footer_ax.text(0.5, 0.18, outcome, ha="center", va="center", fontsize=8.3, color=TEXT, fontweight="bold")
        else:
            footer_ax.text(0.5, 0.66, config["allowed_science_caption_now"], ha="center", va="center", fontsize=8.2, color="#FBBF24", fontweight="bold")
            footer_ax.text(0.5, 0.16, "MECHANISM PROBE QUALIFIED • CHASE STILL DEVELOPMENT-ONLY", ha="center", va="center", fontsize=8.0, color=MUTED, fontweight="bold")
        footer_ax.text(0.99, 0.02, f"replay {bundle['recording_sha256'][:12]}", ha="right", va="bottom", fontsize=6.2, color="#64748B")
        return []
    ...
    try:
        if output.suffix.lower() == ".gif":
            ...
            ani.save(output, writer=animation.FFMpegWriter(fps=fps, bitrate=7600, extra_args=["-pix_fmt", "yuv420p"]))
    finally:
        plt.close(fig)
    return output
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render the evidence-gated Who Farted? rapid mechanism teaser v3")
