...
def _normalize_projection(points: np.ndarray) -> np.ndarray:
    ...
    center = 0.5 * (lo + hi)
    # One uniform scale preserves anatomical aspect ratio and never clips outliers.
    span = max(float((hi - lo).max()), 1.0)
    return 2.0 * (points - center) / span
...
def build_neural_scene(bundle: GraphBundle) -> dict[str, Any]:
    ...
    if not positioned:
        raise ValueError("no finite source soma coordinates available")
    ...
    claim_label = "SOURCE STRUCTURE • NOT A QUALIFIED RESULT"
    return {
        ...
        "projection": "source anatomical x/z coordinates, uniformly scaled; edges are connectivity, not neurites",
        "nodes_total": len(bundle.nodes),
        "all_body_ids": sorted(int(x) for x in bundle.nodes.bodyId),
...
def write_neural_scene(path: str | Path, scene: dict[str, Any]) -> Path:
    ...
    sealed = dict(scene)
    sealed.pop("scene_sha256", None)
    digest = hashlib.sha256(json.dumps(sealed, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()).hexdigest()
    sealed["scene_sha256"] = digest
    output.write_text(json.dumps(sealed, indent=2, sort_keys=True, allow_nan=False) + "\n")
    return output
