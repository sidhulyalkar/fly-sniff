...
from .showcase_v4_layers import (
    blend_activity_color,
    density_rgba,
...
def draw_room_v4(
    ...
    *,
    ...
    recent_steps = max(2, round(recent_seconds / float(payload["dt"])))
...
def draw_population_panel(
    ...
    _, columns, values, scale = pfl3_population_frame(
        e002c,
        fc2,
        threshold=threshold,
    ...
    ax.text(
        0.02,
        0.92,
        "SEALED MODEL PROBE • SEPARATE FROM CHASE TIMELINE",
        color="#FDE68A",
        fontsize=9.1,
        fontweight="bold",
    )
    ax.text(
        0.98,
        0.92,
        "MODELED PFL3 STATE • NOT MEASURED FIRING",
        ha="right",
        color=MUTED,
        fontsize=7.0,
        fontweight="bold",
    ...
    ax.text(
        0.02,
        0.77,
        "FC2 goal interface + direct EPG heading interface → 24-cell PFL3 state",
        color=TEXT,
        fontsize=7.7,
        fontweight="bold",
    )
    ax.text(
        0.98,
        0.77,
        f"E002c 8/8 • w≥{threshold} • probe step {probe_index + 1}/32",
        ha="right",
        color=MUTED,
        fontsize=6.9,
    )
        ...
        for j, value in enumerate(values[name]):
            ...
            ax.add_patch(
                plt.Rectangle(
                    ...
                    x1 - x0,
                    0.10,
                    facecolor=blend_activity_color(color, float(value / scale)),
                    edgecolor="#334155",
                    linewidth=0.35,
                )
            )
    ...
    badges = " • ".join(
        f"w≥{threshold_value}: {changed}/{reachable}"
        for threshold_value, changed, reachable in _threshold_summary(e002c)
    )
    ax.text(
        0.02,
        0.07,
        "joint-changed / dual-reachable  " + badges,
        color=MUTED,
        fontsize=6.4,
    )
    ax.text(
        0.98,
        0.07,
        "FC2 A/B/C: C1-C9 parsed 100% • phase→angle unresolved",
        ha="right",
        color="#C4B5FD",
        fontsize=6.4,
        fontweight="bold",
